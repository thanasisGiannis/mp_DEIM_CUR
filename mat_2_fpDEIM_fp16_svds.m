    ...
    [V,S,W]    = svd(full(A),0); S = diag(S);
    svds_options.format = 'h';
    [Vs,~,Ws]  = my_svds_chop(A,maxK,1e-01,svds_options);
    ...
    ylabel(ax1,'$$||A-\hat{C} U \hat{R}||$$', 'Interpreter', 'latex','FontSize',18);
            ...
            title(ax1,'Finite precision DEIM for fp16 SVDs','Interpreter', 'latex','FontSize',18);
    ...
    ylabel(ax2,'$$\hat{\eta}_p$$', 'Interpreter', 'latex','FontSize',18);
        ...
        title(ax2,'$\hat{\eta}_p$ for finite precision DEIM for fp16 SVDs', 'Interpreter', 'latex','FontSize',18);

saveas(f1,'mat_2_fpDEIM_fp16_svds','pdf')
saveas(f2,'mat_2_eta_DEIM_fp16_svds','pdf')
