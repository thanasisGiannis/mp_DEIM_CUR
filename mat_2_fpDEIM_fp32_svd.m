    ...
    [Vs,~,Ws] = svd(single(A),0);
    Vs = double(single(Vs(:,1:maxK)));
    Ws = double(single(Ws(:,1:maxK)));
    ...
    ylabel(ax1,'$$||A-\hat{C} U \hat{R}||$$', 'Interpreter', 'latex','FontSize',18);
        ...
        title(ax1,'Finite precision DEIM for fp32 SVD','Interpreter', 'latex','FontSize',18);
    ...
    ylabel(ax2,'$$\hat{\eta}_p$$', 'Interpreter', 'latex','FontSize',18);
        ...
        title(ax2,'$\hat{\eta}_p$ for finite precision DEIM for fp32 SVD', 'Interpreter', 'latex','FontSize',18);

saveas(f1,'mat_2_fpDEIM_fp32_svd','pdf')
saveas(f2,'mat_2_eta_DEIM_fp32_svd','pdf')
